...
train_pipeline_LineEX = [
    # Load image from file and convert to float32
    dict(type='LoadImageFromFile', to_float32=True),
    
    # Load annotations with bounding boxes and masks, but without segmentation
    dict(
        type='LoadAnnotations', with_bbox=True, with_mask=True,
        with_seg=False),
    
    # Apply random horizontal and vertical flips with specified probabilities
    dict(
        type='RandomFlip',
        flip_ratio=[0.3, 0.3],
        direction=['horizontal', 'vertical']),
    
    # Resize the image to the specified scale, keeping the aspect ratio
    dict(
        type='Resize',
    ...
    # Pad the image to the specified size with given padding values
    dict(
        type='Pad',
        size=image_size,
        pad_val=dict(img=(255, 255, 255), masks=0, seg=255)),
    
    # Normalize the image using the specified normalization configuration
    dict(
        type='Normalize', **img_norm_cfg),
    
    # Format the image and annotations into the default bundle
    dict(type='DefaultFormatBundle', img_to_float=True),
    
    # Collect the specified keys from the pipeline output
